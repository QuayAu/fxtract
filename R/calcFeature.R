...
#' @param summarize function. If more than one value is returned for each group, this function summarizes these values to one value. E.g. mean.
...
calcFeature = function(data, group_col, fun, check_fun = TRUE, summarize, colname) {
  ...
  if (!missing(summarize)) {
    ...
    res0 = data.frame(apply(res0, 1, function(x) do.call(summarize, list(x))))
    if (!missing(colname)) res0 = res0 %>% setNames(colname)
    res = data.frame(res %>% select(one_of(group_col[1])), res0)
  }
  return(res)
}
