---
title: "R6 Class Project - Utilize batchtools"
...
- Useful for projects with large datasets and many features.
- When some features fail on some datasets, one can get individual error messages.
- Get overview of current calculation status (if feature extraction takes a long time).
...
One can still get the resulting dataframe:
```{r}
my_project$collect_results()
```

## Get Error Messages
This is done by using batchtools functions directly on the project's registry:
...
## Project Status
We have implemented a method `$get_project_status()` which gives an overview over which features have already been calculated on which datasets (1 done, 0 not done):

```{r, message = FALSE}
my_project$get_project_status()
```
...
## Parallelization
Parallelization is done via setting the cluster functions via batchtools. See [here](https://mllg.github.io/batchtools/articles/batchtools.html).
