...
test_that("calc_feature", {
  ...
  expect_error(calc_feature(data = df, group_col = "id", fun = fun))

  #return 1 value
  fun = function(data) c(mean_x = mean(data$x))
  y = calc_feature(data = df, group_col = "id", fun = fun)
  ...
  y = calc_feature(data = df, group_col = "id", fun = more_features)
  expect_equal(y$mean_x, c(mean(1:10), mean(11:20)))
  ...
  #group by 2 columns
  y = calc_feature(data = df, group_col = c("id", "task"), fun = more_features)
  expect_equal(dim(y), c(2, 7))

  #group by 3 columns
  y = calc_feature(data = df, group_col = c("id", "task", "hour"), fun = more_features)
  expect_equal(dim(y), c(2, 3 * 4 + 1))

  #group by 4 columns
  df$y = c(rep(1, 2), rep(2, 2))
  y = calc_feature(data = df, group_col = c("id", "task", "hour", "y"), fun = more_features)
  expect_equal(dim(y), c(2, 3 * 4 * 2 + 1))

  #change colname
  fun = function(data) c(mean_x = mean(data$x))
  y = calc_feature(data = df, group_col = "id", fun = fun, colname = "mean_of_x")
  expect_equal(colnames(y)[2], "mean_of_x")

  #summarize function return 1 value
  y1 = calc_feature(data = df, group_col = c("id", "hour"), fun = fun)
  y2 = calc_feature(data = df, group_col = c("id", "hour"), fun = fun, summarize = mean)
  expect_equal(y2[, 2], rowMeans(y1[, -1]))
  expect_equal(colnames(y2)[2], "mean")

  #summarize function return 1 value with changed colname
  y1 = calc_feature(data = df, group_col = c("id", "hour"), fun = fun)
  y2 = calc_feature(data = df, group_col = c("id", "hour"), fun = fun, summarize = mean, colname = "mean_x")
  expect_equal(y2[, 2], rowMeans(y1[, -1]))
  ...
  y1 = calc_feature(data = df, group_col = c("id", "hour"), fun = fun)
  y2 = calc_feature(data = df, group_col = c("id", "hour"), fun = fun, summarize = summaryFun)
  expect_equal(y2[, 2], rowMeans(y1[, -1]))
